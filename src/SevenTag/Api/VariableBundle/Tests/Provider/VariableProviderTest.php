...
class VariableProviderTest extends \PHPUnit_Framework_TestCase
{
     ...
     * @param $data
     */
    public function itShouldProvideOneVariable($data)
    {
        $provider = new VariableProvider([$data]);

        $this->assertCount(1, $provider->createVariables());
        $this->assertEquals($data['name'], $provider->createVariables()->first()->getName());
    }
    ...
    {
        $parser = new Parser();
        $data = $parser->parse(file_get_contents(__DIR__.'/Fixtures/variables.yml'));
